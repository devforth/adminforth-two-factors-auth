...
export function validatePluginConfig(plugin: any, adminforth: any): void {
  if (plugin.options.passkeys) {
    ...
    if ( !metaField ) {
      const similar = suggestIfTypo(credentialResource.columns.map(c => c.name), plugin.options.passkeys.credentialMetaFieldName);
      throw new Error(
        `Passkeys credentialMetaFieldName '${plugin.options.passkeys.credentialMetaFieldName}' not found in resource '${plugin.options.passkeys.credentialResourceID}'. ${
          similar ? `Did you mean '${similar}'?` : ''
        }`
      );
    }
    ...
    if (plugin.options.passkeys.allowLoginWithPasskeys !== false && !plugin.options.passkeys.settings.rp?.id) {
      throw new Error('Passkeys settings.rp.id is required when allowLoginWithPasskeys is enabled');
    }
