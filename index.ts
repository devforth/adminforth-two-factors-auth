...
export default class TwoFactorsAuthPlugin extends AdminForthPlugin {
  ...
  activate ( resourceConfig: AdminForthResource, adminforth: IAdminForth ){
    ...
    beforeLoginConfirmationArray.push(
      async({ adminUser, response, extra, sessionDuration }: { adminUser: AdminUser, response: IAdminForthHttpResponse, extra?: any, sessionDuration?: string} )=> {
        ...
        const useSessionDuration = sessionDuration ? sessionDuration : '1d'; // default 1 day
        let newSecret = null;
  ...
  setupEndpoints(server: IHttpServer): void {
    ...
    server.endpoint({
      ...
      handler: async ({ body, response, cookies, headers, requestUrl, query }) => {
        ...
        const sessionDuration = this.options.passkeys.rememberDaysAfterPasskeyLogin ? this.options.passkeys.rememberDaysAfterPasskeyLogin : this.adminforth.config.auth.rememberMeDuration;
